...
/*
 ...
 3- We select run s among weights[s] values are max whose have max average score among all max scores.
...
/*
    MTF1_new -> not counting invalid runs in get_avg()
    MTF1_new1 -> s_avg > mx_avg ====> s_avg >= mx_avg
...
void thread_task_MTF1(TOPIC_MTF1& o) {
    o.get_docs();
}

void run_MTF1_for_query(FILE *ofile, int judge_count) {
    ...
    TOPIC_MTF1 topics[UQID_SIZE];
    
    for (int i = 0; i < queries.size(); i++) {
        int qid = queries[i];
        topics[i] = TOPIC_MTF1(qid, ref(ofile), judge_count);
    }
    
    for (int i = 0; i < queries.size(); i++) {
        int qid = queries[i];
        t_qids[qid] = thread(thread_task_MTF1, ref(topics[i]));
    }
